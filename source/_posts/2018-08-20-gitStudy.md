...
## 6. git报错出现 "fatal: detected dubious ownership in repository" 的错误信息

通常是因为 Git 对于该目录的所有权或安全性有疑虑。Git 在版本 2.35.1 中引入了一个新的安全特性，以防止潜在的路径穿越攻击。解决方案如下

```shell
# 当前目录添加，全局生效，可以--local只针对当前git
git config --global --add safe.directory /path/to/git
