...
在Go语言中，结构体选项与默认值的设置有多种方式，下面介绍四种常用的方法：

#### 1. 直接赋值方式

直接赋值是最简单的方式，需要外部调用方获取默认值然后修改。这种方式简单直观，但不够灵活。
...
func main() {
    ...
    fmt.Printf("%+v\n", server)
    // 输出: &{Host:example.com Port:9090 Timeout:30s MaxConn:100}
}
```

#### 2. Builder方式

Builder方式使用链式调用，通过`.WithXXX().WithXXX()`方式设置选项。这种方式灵活且可读性好。
...
func main() {
    ...
    fmt.Printf("%+v\n", server)
    // 输出: &{host:example.com port:9090 Timeout:60s MaxConn:100}
}
```

#### 3. 函数选项模式（Functional Options）

函数选项模式是Go中非常流行的一种方式，new方法内置默认值，然后传入变参opts，在new里面赋值。
...
// Option类型是一个函数，接收Server指针并修改它
type Option func(*Server)

// WithHost返回一个设置host的Option函数
...
// NewServer创建一个新的Server，应用所有提供的选项
...
func main() {
    ...
    fmt.Printf("%+v\n", server)
    // 输出: &{host:example.com port:9090 Timeout:60s MaxConn:200}
}
```

#### 4. 接口选项模式（Interface Options）

接口选项模式是在函数选项模式的基础上，将opts类型从func变成interface，实现apply方法。
...
这种方式对比函数选项式，限制了外部不能自己实现Option接口，只能使用内置的函数选项。
...
func print(r *http.Request) {
	fmt.Println("request host: ", r.Host)				// 172.22.27.203:8081 请求服务端的Host，header里面的host
	fmt.Println("request method: ", r.Method)			// POST
	fmt.Println("request header: ", r.Header)			// map，可以获取对应的header，host不在里面
	fmt.Println("request path: ", r.URL.Path)			// /test/api/v1/client/aio/info/report url不带参数和host
	fmt.Println("request requestURI: ", r.RequestURI)	// /test/api/v1/client/aio/info/report?a=1 url带参数
	fmt.Println("request remoteAddr: ", r.RemoteAddr)	// 172.22.27.203:58438 客户端的ip和端口
}
```

```shell
request host: 172.22.27.203:8081
request method: POST
request header: map[Accept:[*/*] Accept-Encoding:[gzip, deflate, br] Cache-Control:[no-cache] Connection:[keep-alive] Content-Length:[38] Content-Type:[application/json] User-Agent:[PostmanRuntime-ApipostRuntime/1.1.0]]
request path: /test/api/v1/client/aio/info/report
request requestURI: /test/api/v1/client/aio/info/report?a=1
request remoteAddr: 172.22.27.203:58438
```
