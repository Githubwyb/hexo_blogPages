...
# 客户端
openssl s_client -connect 127.0.0.1:7777 -cipher RSA
```
...
Content Type: Handshake(22)
使用客户端DH参数、服务端DH参数生成预主密钥
使用客户端随机数、服务端随机数、预主密钥生成主密钥
使用主密钥发送加密数据
end note
...
## 5. TLS1.3 普通认证流程 TLS_AES_256_GCM_SHA384(0x1302)
...
server->client: Server Hello, Change Chipher Spec, Encrypted Extensions, Certificate, Certificate Verify, Finished
note over server
Server Hello
Content Type: Handshake(22)
Handshake Type: Server Hello(2)
