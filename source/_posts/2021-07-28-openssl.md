...
- `-cert <certPath>`: 证书
- `-CAfile <caPath>`: ca根证书路径，用于校验对端证书
- `-verify int`: 开启校验对端证书
...
openssl.exe s_server -accept 777 -tls1_1 -state -debug -cipher ECC-SM4-SM3 -state -key ./apps/certs/sm2sign.key -cert ./apps/certs/sm2sign.crt -ekey ./apps/certs/sm2enc.key -ecert ./apps/certs/sm2enc.crt -CAfile ./apps/certs/server.crt -Verify 1
# 客户端请求使用skf引擎
