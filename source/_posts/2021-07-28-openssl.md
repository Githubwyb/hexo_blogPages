...
########## rand 生成随机数 ##########
# -hex      按照hex方式输出，不然就是二进制输出，大概率是乱码
# -base64   将二进制按照base64编码后输出
# num       输出几个字节
=> openssl rand -hex 5
f91eca24fa
...
```shell
# 生成ca证书和私钥，有效期365天
# -nodes：表示生成的私钥不加密（no DES），即私钥文件不会被密码保护
openssl req -x509 -nodes -newkey rsa:4096 -keyout cakey.pem -out cacert.pem -days 365
...
openssl req -x509 -nodes -newkey rsa:4096 -keyout cakey.pem -out cacert.pem -days 365 -subj "/C=CN/ST=Beijing/L=aaa/O=ddd/OU=SDT/OU=abc/CN=ddd/emailAddress=aaa@qq.com"
...
# 生成证书请求文件和对应私钥
# -nodes：表示生成的私钥不加密（no DES），即私钥文件不会被密码保护
openssl req -nodes -newkey rsa:4096 -keyout certkey.pem -out cert.csr
# 将有一些配置需要填写，可以随意填写，因为是自颁发
# Country Name (2 letter code) [AU]: 国家名，自然CN
# State or Province Name (full name) [Some-State]: 省份名称，比如Hunan
# Locality Name (eg, city) []: 城市名称，比如changsha
...
# 也可以直接使用-subj指定subject
openssl req -nodes -newkey rsa:4096 -keyout certkey.pem -out cert.csr -subj "/C=CN"
...
# 签名，可以自定义有效天数-days
openssl ca -in cert.csr -cert cacert.pem -keyfile cakey.pem -days 365 -out cert.crt

# 生成p12，需要设置密码
openssl pkcs12 -export -out cert.p12 -inkey certkey.pem -in cert.crt
```

**3. 可以直接生成ca证书用于ssl的服务端证书**

```shell
# 生成匹配域名的证书，将后面的test.com改成自己需要的域名就好
openssl req -x509 -nodes -newkey rsa:4096 -keyout cakey.pem -out cacert.pem -subj "/" -addext "subjectAltName = DNS:calico-api.calico-apiserver.svc"
```
...
- csr证书请求不需要添加拓展，在使用ca签名时才需要添加
- 给csr签名命令中加上`-config /path/to/openssl.cnf -extensions v3_req`
- 或者直接生成证书时指定`-config /path/to/openssl.cnf -extensions v3_req`
...
1. 服务端证书使用本地授信CA验签，说明这个证书是认证的。中间人的代理服务器的证书是不授信的，不过证书是公开的，中间人可能进行重放
2. 预主密钥使用服务端证书的公钥加密，持有私钥的服务端才能解密，说明这个证书确实是服务端持有的。这里代理服务器就无法进行解密了，仅证书持有者才能解密

##### (2) 服务端如何确认客户端是没问题的（防客户端伪造）

1. 客户端提交的证书服务端会使用本地授信的CA验签，说明证书是认证的。中间人的代理服务器的证书是不授信的，不过证书是公开的，中间人可能进行重放
2. 客户端会使用证书私钥将握手数据的签名，服务端使用客户端证书的公钥验签通过，说明证书确实是服务端持有的。这里代理服务器就无法进行签名了，仅证书持有者才能签名
